...
import { ArrowLeft, Camera, ChevronRight, FileText, ImagePlus, Pencil, Plus, Trash2, X } from 'lucide-react-native';
...
type ProfileDraft = { full_name: string; headline: string; location: string; summary: string; skills: string; engineering_highlights: string; published: boolean };
type ProjectDraft = { title: string; slug: string; tagline: string; summary: string; description: string; stack: string; repository_url: string; live_url: string; position: string; featured: boolean; published: boolean };
...
const emptyProfile: ProfileDraft = { full_name: '', headline: '', location: '', summary: '', skills: '', engineering_highlights: '', published: false };
const emptyProject: ProjectDraft = { title: '', slug: '', tagline: '', summary: '', description: '', stack: '', repository_url: '', live_url: '', position: '0', featured: false, published: false };
...
function ProfileEditor({ profile, onSaved }: { profile: PortfolioProfile | null; onSaved: () => Promise<unknown> }) {
  ...
  useEffect(() => {
    setDraft(profile ? {
      ...
      skills: toLines(profile.skills), engineering_highlights: toLines(profile.engineering_highlights), published: Boolean(profile.published),
    } : emptyProfile);
  }, [profile]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      appendText(form, 'portfolio_profile', draft, ['skills', 'engineering_highlights']);
...
function ProjectEditor({ editing, onClose, onSaved }: { editing: PortfolioProject | null | undefined; onClose: () => void; onSaved: () => Promise<unknown> }) {
  ...
  useEffect(() => {
    setDraft(editing ? {
      title: editing.title, slug: editing.slug, tagline: editing.tagline || '', summary: editing.summary, description: editing.description || '', stack: toLines(editing.stack), repository_url: editing.repository_url || '', live_url: editing.live_url || '', position: String(editing.position || 0), featured: Boolean(editing.featured), published: Boolean(editing.published),
    } : emptyProject);
    setCover(null);
  }, [editing]);
  const save = useMutation({
    mutationFn: () => {
      const form = new FormData();
      appendText(form, 'portfolio_project', draft, ['stack']);
...
function SmallButton({ icon: Icon, label, onPress }: { icon: typeof Pencil; label: string; onPress: () => void }) { const theme = useAppTheme(); return <Pressable accessibilityRole="button" onPress={onPress} style={[styles.smallButton, { borderColor: theme.border }]}><Icon color={theme.primary} size={15} /><Text numberOfLines={1} style={[styles.smallLabel, { color: theme.text }]}>{label}</Text></Pressable>; }
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 13, fontWeight: '800', marginBottom: 7 }, field: { borderRadius: 8, borderWidth: 1, fontSize: 15, minHeight: 47, paddingHorizontal: 12, paddingVertical: 11 }, multiline: { minHeight: 104, textAlignVertical: 'top' },
  ...
  projectRow: { alignItems: 'center', borderRadius: 8, borderWidth: 1, flexDirection: 'row', gap: 12, padding: 11 }, projectThumb: { borderRadius: 7, height: 78, width: 78 }, titleLine: { alignItems: 'center', flexDirection: 'row', gap: 8 }, rowTitle: { flex: 1, fontSize: 15, fontWeight: '800' }, rowMeta: { fontSize: 11, marginTop: 4 }, status: { fontSize: 9, fontWeight: '900' }, rowActions: { flexDirection: 'row', gap: 7, marginTop: 9 },
