...
import type { Conversation, Post } from '@/src/api/types';
...
function PostFeed({ posts }: { posts: Post[] }) {
  ...
  return <ScrollView contentContainerStyle={styles.list}>{posts.map((post) => <PostCard key={post.id} post={post} />)}</ScrollView>;
}
