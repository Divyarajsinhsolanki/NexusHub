...
import { BookOpen, Building2, CalendarDays, ChevronRight, ExternalLink, FileText, FolderLock, GraduationCap, Settings, Shield, Sparkles, Users, UserRound } from 'lucide-react-native';
...
type MenuItem = { slug: string; label: string; detail: string; icon: ComponentType<{ color: string; size: number }>; feature?: string };
...
export default function MoreScreen() {
  ...
  const { user } = useAuth();
  return <Screen header={<PageHeader title="More" subtitle={user?.workspace.name || 'Workspace tools'} />}><ScrollView contentContainerStyle={styles.scroll}>{groups.map((group) => {
    const visible = group.items.filter((item) => !item.feature || user?.features?.[item.feature]);
